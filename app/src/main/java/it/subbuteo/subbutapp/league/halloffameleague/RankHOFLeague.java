...
public class RankHOFLeague {

    private Context ctx;

    public RankHOFLeague(Context ctx){
        this.ctx = ctx;
    }

    public Hashtable<String, HallOfFameData> initializedHOFRnkMap(){
        Hashtable<String, HallOfFameData> ht = new Hashtable<>();
        Enumeration<String> enu = Globals.hallFameLeagueMap.keys();
        if(enu != null){
            while(enu.hasMoreElements()){
                String name = enu.nextElement();
                HallOfFameData hof = new HallOfFameData();
                hof.setName(name);
                hof.setImg(Globals.plIMgLeague.get(name));
                hof.setWins(Globals.hallFameLeagueMap.get(name));
                ht.put(name, hof);
            }
        }
        return ht;
    ...
    public boolean isLeagueEnded(){
        ArrayList<MatchData> arr = Objects.requireNonNull(DataRetriever.dataHashMapLeague.get(ctx.getString(R.string.first_final))).getMatchData();
