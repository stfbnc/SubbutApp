...
public class MatchesLeagueAdapter extends RecyclerView.Adapter<MatchesLeagueAdapter.ViewHolder> {
    ...
    public MatchesLeagueAdapter(ArrayList<MatchData> lst, boolean snglMtch) {
        mList = lst;
    ...
    @NonNull
    @Override
    public MatchesLeagueAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ctx = parent.getContext();
        LayoutInflater inflater = LayoutInflater.from(parent.getContext());
        View match_row = inflater.inflate(R.layout.match_layout_tournament_row, parent, false);
        MatchesLeagueAdapter.ViewHolder viewHolder = new MatchesLeagueAdapter.ViewHolder(match_row);
        return viewHolder;
    }

    @Override
    public void onBindViewHolder(@NonNull MatchesLeagueAdapter.ViewHolder holder, int position) {
        if(mList.size() > 0) {
            ...
            if(!pl1.equals(DataRetriever.EMPTY_PLAYER))
                holder.li.setImageDrawable(ContextCompat.getDrawable(ctx, Globals.plIMgLeague.get(pl1)));
            if(!pl2.equals(DataRetriever.EMPTY_PLAYER))
                holder.ri.setImageDrawable(ContextCompat.getDrawable(ctx, Globals.plIMgLeague.get(pl2)));
        }
    }
