...
public class HallOfFameAdapter extends RecyclerView.Adapter<HallOfFameAdapter.ViewHolder> {
    ...
    public HallOfFameAdapter(ArrayList<HallOfFameData> lst) {
        mList = lst;
    }

    @Override
    public HallOfFameAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        context = parent.getContext();
        LayoutInflater inflater = LayoutInflater.from(context);
        View match_row = inflater.inflate(R.layout.ranking_row, parent, false);
        HallOfFameAdapter.ViewHolder viewHolder = new HallOfFameAdapter.ViewHolder(match_row);
        return viewHolder;
    }

    @Override
    public void onBindViewHolder(@NonNull HallOfFameAdapter.ViewHolder holder, int position) {
        if(mList.size() > 0) {
