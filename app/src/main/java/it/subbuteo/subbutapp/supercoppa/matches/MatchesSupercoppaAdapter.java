...
public class MatchesSupercoppaAdapter extends RecyclerView.Adapter<MatchesSupercoppaAdapter.ViewHolder> {
    ...
    public MatchesSupercoppaAdapter(ArrayList<MatchData> lst, boolean snglMtch) {
        mList = lst;
    ...
    @NonNull
    @Override
    public MatchesSupercoppaAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ctx = parent.getContext();
        LayoutInflater inflater = LayoutInflater.from(parent.getContext());
        View match_row = inflater.inflate(R.layout.match_layout_tournament_row, parent, false);
        MatchesSupercoppaAdapter.ViewHolder viewHolder = new MatchesSupercoppaAdapter.ViewHolder(match_row);
        return viewHolder;
    }

    @Override
    public void onBindViewHolder(@NonNull MatchesSupercoppaAdapter.ViewHolder holder, int position) {
        if(mList.size() > 0) {
            ...
            if(!pl1.equals(DataRetriever.EMPTY_PLAYER))
                holder.li.setImageDrawable(ContextCompat.getDrawable(ctx, Globals.plIMgSupercoppa.get(pl1)));
            if(!pl2.equals(DataRetriever.EMPTY_PLAYER))
                holder.ri.setImageDrawable(ContextCompat.getDrawable(ctx, Globals.plIMgSupercoppa.get(pl2)));
        }
    }
