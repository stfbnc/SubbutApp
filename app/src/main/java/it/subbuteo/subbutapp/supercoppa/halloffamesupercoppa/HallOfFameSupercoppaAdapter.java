...
public class HallOfFameSupercoppaAdapter extends RecyclerView.Adapter<HallOfFameSupercoppaAdapter.ViewHolder> {
    ...
    public HallOfFameSupercoppaAdapter(ArrayList<HallOfFameData> lst) {
        mList = lst;
    }

    @NonNull
    @Override
    public HallOfFameSupercoppaAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        context = parent.getContext();
        LayoutInflater inflater = LayoutInflater.from(context);
        View match_row = inflater.inflate(R.layout.ranking_row, parent, false);
        HallOfFameSupercoppaAdapter.ViewHolder viewHolder = new HallOfFameSupercoppaAdapter.ViewHolder(match_row);
        return viewHolder;
    }

    @SuppressLint("SetTextI18n")
    @Override
    public void onBindViewHolder(@NonNull HallOfFameSupercoppaAdapter.ViewHolder holder, int position) {
        if(mList.size() > 0) {
